...
#Collects all files ending with 'properties.csv' in the script's directory
#and moves them to the specified destination folder within the same directory.
def collect_and_move_properties_files(destination_folder):
   
    #Determining the Script's Directory
    script_directory = os.path.dirname(os.path.abspath(__file__))
    destination_directory = os.path.join(script_directory, destination_folder)
    ...
    for filename in os.listdir(script_directory):
        if filename.endswith("properties.csv"):
...
if __name__ == "__main__":
    ...
    # Collect all files that end with properties.csv in the same folder, so they can be imported faster.
    properties_destination_folder = 'CollectedPropertiesFiles'
    collect_and_move_properties_files(properties_destination_folder)